...
pinterest = Pinterest(email="EvaLDelafuente@outlook.com",
                      password="Savings$2022",
                      username="evaldelafuente",
                      cred_root='cred_root'
                      #,proxies=proxies
                      )
...
def first_interaction(username):
    conn = psycopg2.connect(host="ec2-18-235-86-66.compute-1.amazonaws.com", port = 5432, database="d3v390l92675i2", user="doqzivchzhzmqf", password="3a2e2b852b948b0f3a11d8d3346f6b5aa54c599ac9c12dcaf5b3d741a20cccb2")
    cur = conn.cursor()
    boards_list_info=[]

    file1 = open("boards_evaldelafuente.txt","r")
...
def first_related_pins_pinning(board_name,board_id,username):
    ...
    i=1
    p=0
    range_interval=random.randint(50,70)
    n=1
    key=0
    ...
    while i <= len(selected) and p <= range_interval:
        ...
        if (i == 1):
            own_search_query = "deembeam " + board_name
            time.sleep(random.randint(15,25))
            results_own_pins = search(10, 'pins', own_search_query)
            print("------------------------------------------------------------")
            daily_interaction_file = open("dailypins_evaldelafuente.txt", "r")
            ...
            for i in range(len(list)):
                if i % 2 == 0:
                    ...
                    print(f" Going to take between 10 to 15 seconds. I'm so tired !!!")
...
first_interaction(username='evaldelafuente',)

boards_res=pinterest.boards_all(username='evaldelafuente')
